    ...
    <div className='w-full h-auto flex items-center justify-center flex-col bg-gray-100 max-md:py-10'>
      ...
      {products.map((product)=>(
        <div className='w-64 h-[344px] bg-gray-500 flex flex-col items-center justify-start rounded-2xl m-auto' key={product.id}>
            ...
            <button className='w-[235px] h-10 mt-2 bg-[#b91c1c] text-xl rounded-lg font-semibold cursor-pointer' onClick={() => addToCart(product.id, 'allProducts')}>Add to Cart</button>
        </div>
       ))} 
      </div> 
      </div> 
