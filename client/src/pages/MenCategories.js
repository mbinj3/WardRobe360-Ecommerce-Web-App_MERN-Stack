...
function MenCategories() {
    ...
    const {addToCart, menProducts} = useContext(addCounterContext)

  return (
    <div className='w-full h-auto flex items-center justify-center flex-col bg-gray-100 pt-20'>
      <p className='text-center text-2xl px-3.5 py-0.5 border-2 border-black text-[#b91c1c] rounded-full font-semibold  tracking-wide'>Men Shirts</p>
      ...
      {menProducts.map((product)=>(
        <div className='w-64 h-[344px] bg-gray-500 flex flex-col items-center justify-start rounded-2xl m-auto' key={product.id}>
            ...
            <button className='w-[235px] h-10 mt-2 bg-[#b91c1c] text-xl rounded-lg font-semibold cursor-pointer' onClick={() => addToCart(product.id, 'men')}>Add to Cart</button>
        </div>
       ))} 
      </div> 
      </div> 
