...
function ShowsCategories() {
    ...
    const {addToCart, accessoriesProducts} = useContext(addCounterContext)

  return (
    <div className='w-full h-auto flex items-center justify-center flex-col bg-gray-100 pt-20'>
      <p className='text-center text-2xl px-3.5 py-0.5 border-2 border-black text-[#b91c1c] rounded-full font-semibold  tracking-wide'>Men Accessories</p>
      ...
      {accessoriesProducts.map((product)=>(
        ...
        <button className='w-[235px] h-10 mt-2 bg-[#b91c1c] text-xl rounded-lg font-semibold cursor-pointer' onClick={() => addToCart(product.id, 'accessories')}>Add to Cart</button>
        </div>
       ))} 
      </div> 
      </div> 
